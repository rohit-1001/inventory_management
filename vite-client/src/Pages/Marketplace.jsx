...
const Marketplace = () => {
  ...
  const handleProductSelect = async (e) => {
    ...
    if (product) {
      ...
      try {
        const response = await axios.get(`api/getPrice?productName=${encodeURIComponent(productName)}`);
  ...
  async function handleClickOpen(company) {
    try {
      const c = await axios.post("api/getSelectedCompany", {email : company.email}, {withCredentials: true})
  ...
  const handleOrderSubmit = async () => {
    if (selectedProducts.length !== 0) {
      try {
        const c = await axios.post("api/request", {
