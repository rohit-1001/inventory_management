...
export default function OrderTable(props) {
  ...
  const confirmDelivery = async (id) => {
    try {
      const dataaa = await axios.post("api/getcurrorderinfo", { id }, { withCredentials: true });
    ...
    try {
      const c = await axios.post("api/confirmDelivery", { id }, { withCredentials: true });
      if (c.status === 200) {
        toast.success(c.data.msg);
      }
    } catch (error) {
