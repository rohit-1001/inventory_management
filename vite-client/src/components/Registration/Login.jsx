...
const Login = (props) => {
  ...
  const loginForm = async (event) => {
    ...
    if (role.value === "company") {
      try {
        const res = await axios.post("api/companysignin", userData);
        if (res.status !== 200) {
      ...
      try {
        const res = await axios.post("api/vendorsignin", userData);
        if (res.status !== 200) {
          toast.error(res.data.msg);
        } else {
          toast.success(res.data.msg);
  ...
  const signupForm = async (event) => {
    ...
    if (role.value === "company") {
      try {
        const res = await axios.post("api/companyregister", userDataSignUp);
      ...
      try {
        const res = await axios.post("api/vendorregister", userDataSignUp);
